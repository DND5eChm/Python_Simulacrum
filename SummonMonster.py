...
class Monster:
    def __init__(self, content):
        ...
        self.speed = self.find_data_line(content_lines,["速度","speed"],"pattern")
        ...
        self.resistance = self.find_data_line(content_lines,["抗性","resistance"])
        ...
        for line in content_lines:
            for prefix in data_prefixs:
                if line.lower().startswith(prefix): #开头匹配，最优的情况
                    ...
                    if line[left+len(prefix)] in [" ",":","："]: #但依旧完美匹配，加入可能列表
                    ...
                    for char in action_name:
                        ...
                        if (not pattern_now) and (char.isdigit() or char == " " or char.encode().isalpha()):
                            ...
                            if not pattern_now and en_now:
                                ...
                                en_now = False
                    if cnwords >= 2 and cnwords <= 8 and enwords >= 3:#动作英文里最短的Ram都有3个字,最短的中文是2个字,最长的中文都只有8个字
                        if sus: #你很可疑，我得再检查一下
                            for sus_word in ["可以","长休","短休","使用次数","目标","必须","否则","失败","成功"]:
                                result = content_line
                            if result == "": #诶你小子真的这个神经病结构啊
                                result = template_actionlabel.replace("{{名称}}",action_name).replace("{{内容}}",content_line[right+1:])
